<div class="flex h-screen bg-gray-100 gap-0" x-data="{ showStruk: false }">
    ...
    <div class="flex-1 flex flex-col p-4 overflow-hidden">
        ...
        <div class="relative mb-4">
            <input
                ...
                class="w-full pl-10 pr-4 py-3 border rounded-xl text-sm focus:ring-2 focus:ring-blue-500"
                autofocus
            />
        ...
        <div class="absolute z-50 top-32 left-4 right-4 bg-white rounded-xl shadow-lg border max-h-72 overflow-y-auto">
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 border-b">
                    <tr>
                        ...
                        <th class="text-center px-4 py-3 text-gray-600 font-medium w-28">Qty</th>
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-2">
                                <button wire:click="updateQty({{ $productId }}, {{ $item['qty'] - 1 }})"
                                    class="w-7 h-7 rounded-full bg-gray-100 hover:bg-red-100 flex items-center justify-center text-lg font-bold text-gray-600">−</button>
                                <span class="w-8 text-center font-medium">{{ $item['qty'] }}</span>
                                <button wire:click="updateQty({{ $productId }}, {{ $item['qty'] + 1 }})"
                                    class="w-7 h-7 rounded-full bg-gray-100 hover:bg-green-100 flex items-center justify-center text-lg font-bold text-gray-600">+</button>
                            </div>
                        </td>
    ...
    <div class="w-80 bg-white shadow-lg flex flex-col p-5 gap-4">
        <h2 class="font-bold text-gray-800 text-lg border-b pb-3">Ringkasan Transaksi</h2>
        ...
        <div>
            ...
            <div class="grid grid-cols-2 gap-2">
                ...
                <label class="flex items-center gap-2 cursor-pointer border rounded-lg px-3 py-2 text-sm
                    {{ $paymentMethod === $val ? 'border-blue-500 bg-blue-50 text-blue-700' : 'text-gray-600' }}">
        ...
        <button
            ...
            class="mt-auto w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-xl text-lg transition disabled:opacity-50"
        >
            <span wire:loading.remove>Proses Pembayaran</span>
            <span wire:loading>Memproses...</span>
        </button>

        <button wire:click="clearCart" class="w-full bg-gray-100 hover:bg-gray-200 text-gray-600 font-medium py-2 rounded-xl text-sm transition">
            Batal / Kosongkan
        </button>
    </div>
</div>